...
%         pri_vis_cortex_beta=sum(pri_vis_cortex_beta(:))/length(find(pri_vis_cortex_mask~=0));
        ...
        bm_pri_audio_cortex_34{i,j}=pri_audio_cortex_beta(pri_audio_cortex_beta~=0);%%%%%%%%
%         pri_audio_cortex_beta=sum(pri_audio_cortex_beta(:))/length(find(pri_audio_cortex_mask~=0));
        ...
        bm_pri_motor_cortex_34{i,j}=pri_motor_cortex_beta(pri_motor_cortex_beta~=0);%%%%%%%%
%         pri_motor_cortex_beta=sum(pri_motor_cortex_beta(:))/length(find(pri_motor_cortex_mask~=0));
        ...
        pri_vis_cortex_34(i,j)        = pri_vis_cortex_beta;
        vis_cortex_34(i,j)            = vis_cortex_beta;
        out_vis_cortex_34(i,j)        = out_vis_cortex_beta;
        pri_audio_cortex_34(i,j)      = pri_audio_cortex_beta;
        out_pri_audio_cortex_34(i,j)  = out_pri_audio_cortex_beta;
        pri_motor_cortex_34(i,j)      = pri_motor_cortex_beta;
        out_pri_motor_cortex_34(i,j)  = out_pri_motor_cortex_beta;


    end
end
pri_vis_cortex_34(isnan(pri_vis_cortex_34))=0;
vis_cortex_34(isnan(vis_cortex_34))=0;
out_vis_cortex_34(isnan(out_vis_cortex_34))=0;
pri_audio_cortex_34(isnan(pri_audio_cortex_34))=0;
out_pri_audio_cortex_34(isnan(out_pri_audio_cortex_34))=0;
pri_motor_cortex_34(isnan(pri_motor_cortex_34))=0;
out_pri_motor_cortex_34(isnan(out_pri_motor_cortex_34))=0;

%% 画图
figure;
errorbar(1:6,mean(test_35),std(test_35)/length(test_35),'LineWidth',1.5);
hold on
errorbar(1:6,mean(test_34),std(test_34)/length(test_34),'LineWidth',1.5);
hold on
errorbar(1:6,mean(test_tp),std(test_tp)/length(test_tp),'LineWidth',1.5);
xticks(1:6);
